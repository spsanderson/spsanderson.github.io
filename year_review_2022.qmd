...
Now that we have our data, we have it for the year 2022 only with a start date of `r min(data_tbl$date)` and an end date of `r max(data_tbl$date)`.
...
There was a total of `r nrow(readRDS("00_data/mapping_dataset.rds"))` different countries that downloaded `healthyverse` packages in 2022.
