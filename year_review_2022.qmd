...
#
> __2022__ A Year in Review

The year 2022 was a big year for me. I did a lot of coding, a lot more than I 
...
In order to start looking at some of the data that pertains to 2022 lets first
...
Ok now that we have our data, lets ensure that we are only using the year 2022.
...
Now that we have our data, we have it for the year 2022 only with a start date
...
So all in all there was a total of `r nrow(data_tbl) %>% scales::number(big.mark=",")` downloads of all the `healthyverse` packages in 2022. There were in total `r total_number_of_releases` package releases as well.
...
countries that downloaded `healthyverse` packages in 2022.
